...
export class Character implements Tickable {
  ...
  private runAnimKeys: Array<string> = [
    "nrun",
    "nerun",
    "erun",
    "serun",
  ...
  private idleAnimKeys: Array<string> = [
    "nidle",
    "neidle",
    "eidle",
    "seidle",
  ...
  constructor(
    ...
    this.sheet =
      PIXI.Loader.shared.resources[
        "src/resources/images/character.json"
      ].spritesheet;

    this.sprite = new PIXI.AnimatedSprite(
      this.sheet.animations[this.currAnimKey]
    );
  ...
  processMovement = (direction: Direction, delta: number) => {
    switch (direction) {
      case Direction.N:
        this.sprite.y -= this.options.speed * this.speedModifier * delta;
        break;
      case Direction.NE:
        this.sprite.y -= this.options.speed * this.speedModifier * delta;
  ...
  update(delta: number) {
    ...
    if (!hasInput) {
      if (!this.currAnimKey.includes("idle")) {
        this.currAnimKey = directionAnimationSwitch(
          this.direction,
          this.idleAnimKeys
        );
